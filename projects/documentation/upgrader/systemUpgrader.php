...
class systemUpgrader {
    ...
    public static function preUpgrade($model, $subSet) {
        ...
        $ret = ($ver);
        if (!$ver) {
            //preguntamos por la versión 0, corresponde con:
            //   {"state":{"generated":true},"main":{"version":1}}
            //   Aquí el valor de "version" se refiere a la versión de fields
                ...
                foreach ($v_config as $key => $value) {
                    if (is_array($value)) {
                        foreach ($value as $k => $v) {
                            $v_project[$key][$k] = $v;  //asignamos a la versión del template el valor actual (no hay valor antiguo)
                        }
                    }else {
                        $v_project[$key] = $ver; //asignamos a fields el valor antiguo
                    }
                }
            }
